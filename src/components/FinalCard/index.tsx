...
class FinalCard extends Component<IProps, IState> {
  ...
  render() {
    return (
      <View>
        <View style={{alignItems: 'flex-start', marginVertical: 10, width: '100%', paddingHorizontal: 30, minHeight: 20}}>
        ...
        <View style={styles.TextContainer}>
          <CheckBox checked={!!this.props.checkedPlaces.filter(val => val.placeId === this.props.place.place_id).length} onPress={this.handlePress} color="green"></CheckBox>
          <View style={{flex: 1, alignItems: 'flex-start', marginLeft: 20}}>
            <AppText style={styles.Text}>{this.props.place.name}</AppText>
          </View>
          <View style={{paddingHorizontal: 10, minWidth: 70}}>
            {this.props.checkedPlaces.filter(val => val.placeId === this.props.place.place_id).length ? <AppText>1h40m</AppText> : <View></View>}
          </View>
        </View>
      </View>
    );
